...
def print_diagnostics():
	...
	try:
		input_devices = audio_manager.get_available_input_devices()
		if input_devices:
			cprint("Доступные устройства ввода:", color="yellow")
			for idx, name, ch in input_devices:
				cprint(f"  [{idx}] {name} (каналов: {ch})", color="bright_black")
		else:
			cprint("Нет доступных устройств ввода!", color="red")
		...
		if default_output != -1:
			...
			cprint("Устройство вывода не определено", color="red")
	except Exception as e:
		error(f"Ошибка диагностики: {e}")
	print()
...
def main():
	...
	if not input_devices:
		error_msg = (
			...
			"Подключите микрофон и перезапустите MikeTyping."
		)
		error(error_msg)
		from src.console_utils import show_error_dialog

		show_error_dialog("MikeTyping: нет микрофона", error_msg)
		return  # завершаем программу

	# Показываем пользователю, какое устройство будет использовано
	chosen_index, chosen_name, chosen_channels = input_devices[0]
	info(
		f"Будет использовано устройство ввода: [{chosen_index}] {chosen_name} ({chosen_channels} канал(ов))"
	)
	...
	try:
		with audio_manager.start_audio_stream():
			info(
				f"Реальные параметры аудиопотока: {audio_manager.ACTUAL_CHANNELS} канал(ов), {audio_manager.ACTUAL_SAMPLE_RATE} Гц"
			)
			while True:
				...
				if prev_recording_active and not recording_active:
					# Небольшая дополнительная пауза для завершения декодирования в ASR
					wait_until = FINALIZATION_WAIT_MS / 1000.0
